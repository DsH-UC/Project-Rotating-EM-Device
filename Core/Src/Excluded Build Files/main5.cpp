...
#include "IRBreakSensor.hpp"
#include "IO_Handler.hpp"
#include "RotatingBlock.hpp"
#include "ChargingStage.hpp"

#include <Testing/CoolingSystemTest.hpp>
#include "Testing/RotatingBlockTest.hpp"
#include "Testing/ChargingStageTest.hpp"
...
int main(void)
{
  ...
  //r.initThermistorThread(1024, tskIDLE_PRIORITY + 1);
  //ir.init(GPIOB, GPIO_PIN_4, GPIOB, GPIO_PIN_5);
  //ir.init_IR_break_thread(256, tskIDLE_PRIORITY + 1);
  cooling_subsystem.init_fan_gpio(GPIOC, GPIO_PIN_5, GPIO_PIN_SET);
  ...
  ChargingStageTest::init_charging_stage_test_thread(256, tskIDLE_PRIORITY + 1);

  arm_pid_instance_f32 pid;
  arm_pid_init_f32(&pid, 1);
  arm_pid_f32(&pid, 5.0f);
  float32_t iirCoefficients[5*1] = {
	0.03f, 0.1f, 0.08f, 0.8f, 0.3f
  };

  float32_t iirStateBuffer[4 * 1];
  arm_biquad_casd_df1_inst_f32 filter;
  arm_biquad_cascade_df1_init_f32(&filter, 1, iirCoefficients, iirStateBuffer);
